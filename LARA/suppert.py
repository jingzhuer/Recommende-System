...
neg=np.array(pd.read_csv('neg_data.csv'))
train_data=np.array(pd.read_csv('data/train_data.csv',usecols=['userId', 'itemId', 'gerne']))
test_data=pd.read_csv('data/test_data.csv',usecols=['itemId','gerne'])
test_data.drop_duplicates(keep='first',inplace=True) #删除重复项
test_data=np.array(test_data)
user_emb_matrix=np.array(pd.read_csv('util/user_emb.csv',header=None))
user_attribute_matrix=np.array(pd.read_csv('util/user_attribute.csv',header=None))
ui_matrix=np.array(pd.read_csv('util/train_ui_matrix.csv'))
